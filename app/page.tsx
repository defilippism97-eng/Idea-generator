...
export default function Home() {
  ...
  function renderMessage(m: Message, i: number, isLast: boolean) {
    ...
    return (
      <div
        key={i}
        className={`animate-fade-in-up flex gap-2.5 ${m.role === "user" ? "ml-auto max-w-[85%] flex-row-reverse" : "max-w-[92%]"}`}
      >
        <Avatar role={m.role} />
              ...
              m.role === "user"
                ...
                : m.role === "persona"
                  ...
                  : m.role === "stakeholder"
                    ...
                    : m.variant
                      ? "border border-emerald-500/20 bg-emerald-500/[0.05] text-neutral-100"
  ...
  return (
    <div className="relative mx-auto flex h-dvh max-w-3xl flex-col">
        ...
        <div className="flex gap-1.5">
          {phase === "chat" && (
            <button
              ...
              className="rounded-lg border border-white/10 px-2.5 py-1.5 text-xs text-neutral-300 transition-colors hover:border-white/20 hover:bg-white/5 hover:text-white"
            >
              + Nuova
            </button>
          )}
      ...
      {archiveOpen && (
        <div className="animate-fade-in absolute inset-x-0 top-16 z-10 mx-auto max-w-3xl border-b border-white/[0.06] bg-[var(--background-elevated)]/95 px-6 py-4 shadow-2xl backdrop-blur-xl">
              ...
              {archiveList.map((item) => (
                <li key={item.id}>
                  <button
                    ...
                    className="flex w-full items-center justify-between gap-2 rounded-lg px-2.5 py-2 text-left text-xs transition-colors hover:bg-white/5"
                  >
      ...
      {phase === "wizard" ? (
        ...
        <>
          ...
          <footer className="border-t border-white/[0.06] px-5 py-4 sm:px-6">
            {!hasFull && (
              <div className="flex gap-2">
                <textarea
                  className="flex-1 resize-none rounded-xl border border-white/10 bg-white/[0.03] px-3.5 py-2.5 text-sm outline-none transition-colors placeholder:text-neutral-500 focus:border-indigo-500/50 focus:bg-white/[0.05]"
                  rows={2}
                ...
                <button
                  ...
                  className="rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 px-4 py-2 text-sm font-medium text-white shadow-lg shadow-indigo-950/50 transition-transform hover:scale-[1.03] active:scale-[0.97] disabled:pointer-events-none disabled:opacity-40"
                >
                  Invia
                </button>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {!hasSummary && (
                  <button
                    ...
                    className="rounded-lg border border-white/10 px-3 py-1.5 text-xs font-medium text-neutral-200 transition-colors hover:bg-white/5 disabled:opacity-30"
                  >
                ...
                {hasSummary && !hasFull && (
                  <button
                    ...
                    disabled={loading}
                    className="rounded-lg border border-emerald-500/25 bg-emerald-500/[0.06] px-3 py-1.5 text-xs font-medium text-emerald-300 transition-colors hover:bg-emerald-500/15 disabled:opacity-30"
                  >
